...
$pembelian_id = $_SESSION['ppembelian_id'];
...
try {
    ...
    $sql = "SELECT ROW_NUMBER() OVER(ORDER BY beli_detail_id asc) AS no, a.beli_detail_id,
                b.nama_produk, b.harga_jual,a.qty as jumlah, b.harga_jual*a.qty as sub_total FROM pembelian_detail a, produk b
            where a.produk_id = b.produk_id
            and a.pembelian_id = :pembelian_id  LIMIT $paginationStart, $limit";
    $stmt = $conn->prepare($sql);
    $stmt->execute($arrData);
    $data = $stmt->fetchAll();

    // Get total records
    $stmt = $conn->prepare("SELECT count(beli_detail_id) AS id FROM pembelian_detail where pembelian_id = :pembelian_id");
    $stmt->execute($arrData);
...
<head>
    ...
    <title>Transaksi Detail Pembelian</title>
</head>
                    ...
                    <div class="forms_content_box">
                        ...
                        <div class="forms_content_box_toolbar">
                            <div class="forms_content_box_toolbar_item">
                                <!-- <button id="iBtnTambah" type="submit" class="btn btn-default btn-sm" style="background-color:#5074dc;color:white;font-size:13px">
                                ...
                                <form action="adm_buy.php" method ="POST">
                                    ...
                                    <button name="BtnViewPenjualan" id="BtnViewPenjualan" type="submit" class="btn btn-default btn-md" style="margin-left:10px;padding-right:30px;background-color:#5074dc;color:white;font-size:14px;">
                                        <span class="fa fa-arrow-left" style="padding-right:10px;"></span> History Pembelian 
                                    </button>
                                </form>
                            </div>
                                ...
                                <table id="tables" class="table table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Produk</th>
                                            <th style="width:50px;text-align:center;">Harga Beli</th>
                                    ...
                                    <ul class="pagination justify-content-center">
                                        ...
                                        <?php for ($i = 1; $i <= $totoalPages; $i++) : ?>
                                            <li class="page-item <?php if ($page == $i) {
                                                ...
                                                <a class="page-link" style="font-size:13px;" href="adm_buy_view.php?page=<?= $i; ?>"> <?= $i; ?> </a>
                                            </li>
                                        <?php endfor; ?>
