...
$stmt = $conn->prepare("SELECT ROW_NUMBER() OVER(ORDER BY a.penjualan_id asc) AS no, a.penjualan_id,
DATE_FORMAT(a.tanggal_penjualan, '%d-%m-%Y') tanggal,TIME_FORMAT(a.created_at, '%H:%i:%s') waktu,
case when a.pelanggan_id  = 0 then 'Bukan Pelanggan' else (select nama_pelanggan from pelanggan where pelanggan_id = a.pelanggan_id) end nama_pelanggan,
(select sum(qty) from penjualan_detail where penjualan_id = a.penjualan_id) total_item,
total total_harga
FROM penjualan a LIMIT $paginationStart, $limit");
$stmt->execute();
$data = $stmt->fetchAll();


// Get total records
$stmt = $conn->prepare("SELECT count(penjualan_id) AS id FROM penjualan");
$stmt->execute();
...
if (isset($_POST['hBtnSimpan'])) {
    ...
    $penjualan_id = $_POST['hpenjualan_id'];
    ...
    try {
        ...
        $sql = "delete from penjualan where penjualan_id = :penjualan_id";
...
if (isset($_POST['BtnDetail'])) {
    ...
    $_SESSION['ppenjualan_id'] = $penjualan_id;

    // echo "ID " . $penjualan_id;
    header("Location: adm_sale_view.php");
}


if (isset($_POST['iBtnTambah'])) {
    header("Location: adm_sale_add.php");
}
...
<script>
    $(document).ready(function() {
        ...
        $("#tables").on('click', '#BtnHapus', function(event) {
            ...
            var penjualan_id = $(this).closest('tr').find('td:eq(1)').text();
            document.getElementById('hpenjualan_id').value = penjualan_id;
...
<body>
                        ...
                        <div class="forms_content_box_toolbar">
                            <div class="forms_content_box_toolbar_item">
                                <form action="" method="POST">
                                    <button name="iBtnTambah" type="submit" class="btn btn-default btn-md" style="padding-right:30px;background-color:#5074dc;color:white;font-size:14px;">
                                        <span class="fa fa-plus" style="padding-right:10px;"></span> Input Penjualan
                                    </button>
                                </form>
                            </div>
                                    ...
                                    <tbody id="tableCariUser">
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td hidden><?php echo $penjualan_id; ?></td>
                                                ...
                                                <td style="text-align: center;">
                                                        <!-- <div style="display:flex;flex-direction:row;"> -->
                                                            <form action="" method="POST">
                                                            <input type="text" hidden name="id" value=<?php echo  $row['penjualan_id'] ?> />
                                    ...
                                    <ul class="pagination justify-content-center">
                                        ...
                                        <?php for ($i = 1; $i <= $totoalPages; $i++) : ?>
                                            <li class="page-item <?php if ($page == $i) {
                                                ...
                                                <a class="page-link" style="font-size:13px;" href="adm_sale.php?page=<?= $i; ?>"> <?= $i; ?> </a>
                                            </li>
                                        <?php endfor; ?>
    ...
    <!-- FORM HAPUS -->
    <form action="adm_sale.php" method="POST">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div>
                            <input type="hidden" id="hpenjualan_id" name="hpenjualan_id">
                        </div>
